...
rstSecond.Open "SELECT Top(1) * FROM BulkSimsAS", MM_Site_STRINGWrite, 1, 2
rstSecond.AddNew
...
RecNewest.Source = "SELECT Top(1)* FROM BulkSimsAS Where Token = '" & ThisToken & "'"
...
RecChecking.Source = "SELECT Top(1)* FROM BulkSimChildren Where SerialNo = '" & RecBrick.Fields.Item("SerialNo").Value & "'"
RecChecking.CursorType = 0
RecChecking.CursorLocation = 2
RecChecking.LockType = 3
RecChecking.Open()
RecChecking_numRows = 0
If Not RecChecking.EOF and not RecChecking.BOF Then
DoInsert = "No"
End If
...
Set conMain = Server.CreateObject ( "ADODB.Connection" )
Set rstSecond = Server.CreateObject ( "ADODB.Recordset" )
rstSecond.Open "SELECT Top(1) * FROM BulkSimChildrenAS", MM_Site_STRINGWrite, 1, 2
rstSecond.AddNew
...
set rstSecond = nothing	
LC = LC + 1
Response.write("<br>" & LC & ". " & RecBrick.Fields.Item("SerialNo").Value)
...
RedirectLink = "ASView.asp?ASID=" & ASID & "&Item=7"
