...
RecUpdateFNBTrans.Open "SELECT Top(1) * FROM MChargeFNBTrans", MM_Site_STRINGWrite, 1, 2
...
RecNewsestFNBTrans.Source = "SELECT Top(2)* FROM MChargeFNBTrans Where TediID = '" & Request.Form("TID") & "' Order By FNBID Desc"
...
RecUpdateFNBTable.Open "SELECT Top(1) * FROM TediTransactions", MM_Site_STRINGWrite, 1, 2
...
RecNewCalcZonerMCharge.Source = "SELECT * FROM ViewMchargeTediTotalAllocated Where TediID = " & Request.Form("TID")
...
RecNewCalcZonerBanked.Source = "SELECT * FROM ViewMchargeTediTotalBanked Where TediID = " & Request.Form("TID")
