...
RecFindZonerTrans.Source = "SELECT * FROM TediTransactions Where TediID = " & ZID & " and CComments = '" & TransDescrition & "'  and CAmount = '" & MChargeValue & "'"
...
While Not RecFindZonerTrans.EOF
		...
		RecUpdateBulk.Open "SELECT Top(1)* FROM TediTransactions Where CID = " & RecFindZonerTrans.Fields.Item("CID").Value, MM_Site_STRING, 1, 2
...
Wend
		...
		conMain.CommandTimeout = DBCommandTimeout
		Set RecUpdateBulk = Server.CreateObject ( "ADODB.Recordset" )
		RecUpdateBulk.Open "SELECT Top(1)* FROM BulkMChargeChildren Where ChildID = " & CID, MM_Site_STRING, 1, 2
		RecUpdateBulk.Update
...
Wend
		...
		conMain.CommandTimeout = DBCommandTimeout
		Set RecUpdateBulk = Server.CreateObject ( "ADODB.Recordset" )
		RecUpdateBulk.Open "SELECT Top(1)* FROM  BulkMCharge Where BulkID = " & WhichBulkID, MM_Site_STRING, 1, 2
		RecUpdateBulk.Update
