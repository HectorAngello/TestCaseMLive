...
RecFindZonerTrans.Source = "SELECT Top(1)* FROM TediTransactionsMM Where TediID = " & ZID & " and (CComments = '" & TransDescrition & "' or CComments = '" & TransDescritionEVD & "')  and CAmount = '" & MChargeValue & "'"
...
rstSecond.Source = "Delete FROM TediTransactionsMM Where CID = " + Replace(RecFindZonerTrans.Fields.Item("CID").Value, "'", "''") + ""
...
Msg = "Mobile Money Reversal of R " & FormatNumber(MChargeValue,2) & " allocated to " & RecZID.Fields.Item("TediFirstName").Value & " " & RecZID.Fields.Item("TediLastName").Value  & " on " & TransDate & ", Regards PMG Live"
...
RecNewCalcZonerMCharge.Source = "SELECT Top(1)* FROM ViewMchargeTediTotalAllocatedMM Where TediID = " & ZID
...
RecNewCalcZonerBanked.Source = "SELECT Top(1)* FROM ViewMchargeTediTotalBankedMM Where TediID = " & ZID
...
Set rstSecond = Server.CreateObject ( "ADODB.Recordset" )
rstSecond.ActiveConnection = MM_Site_STRINGWrite
rstSecond.Source = "Delete FROM BulkMChargeChildrenMM Where ChildID = " + Replace(CID, "'", "''") + ""
rstSecond.Open
set rstSecond = nothing	
