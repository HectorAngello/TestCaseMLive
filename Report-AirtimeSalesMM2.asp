...
SaveFileName = "MobileMoneySales_Report-" & Day(Now) & Month(Now) & Year(Now) & "-" & Hour(Now) & Minute(Now) & Second(Now) & ".csv"
TableHead = "Agent Code,  First Name, Last Name, " & SupervisorLabel & ", Region, Sub Region, Mobile Money"
...
If OutFormat = "B" Then
%>
        <h3>Sales Report Mobile Money</h3>
...
RecTotalAgents.Source = "SELECT Sum(CAmount) AS AgentTotal FROM viewTediTransactionsMM WHERE TID = " & RecAgentEdit.Fields.Item("TID").Value & " and (CDate BETWEEN '" & StartDate & "' AND '" & EndDate & " 23:59:59') and CType = '2'"
