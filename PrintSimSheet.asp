...
RecBulkV.Source = "SELECT Top(1) * FROM BulkSims Where BulkID = " & Request.QueryString("BulkID") 
...
RecBulkChildren.Source = "SELECT * FROM BulkSimChildren Where BulkID = " & Request.QueryString("BulkID") & " Order By ChildCreationDate Asc"
...
RecTedi.Source = "SELECT * FROM ViewTediDetailWithTotals where  TID = " & RecBulkV.Fields.Item("TID").Value
  ...
  <tr>
    <td><table width="700" border="0" cellpadding="1" cellspacing="1">
      <tr>
        ...
        <td colspan="2" align="center" bgcolor="#FFFFFF" width="500" Valign="top"><span class="style5"><h2><%=AgentLabel%> Sim Allocation Sheet</h2>
	<h3>Agent: <%=RecTedi.Fields.Item("TediEmpCode").Value%><br><%=RecTedi.Fields.Item("TediFirstName").Value%>&nbsp;<%=RecTedi.Fields.Item("TediLastName").Value%><br><br>Printed Date:<br><%=FormatDateTime(Now(),1)%><br><br>SIM Capture Date:<br><%=FormatDateTime(RecBulkV.Fields.Item("BulkDate").Value,1)%></h3></span>
	</td>
      </tr>
...
<td width="33%"><span class="style5">------------------------------------<br><%=Session("AgentLabel")%> Signature</span></td>
</tr>
	</table>
	</td>
        </tr>
