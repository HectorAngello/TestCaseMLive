...
RecZoner.Open()
RecZoner_numRows = 0
ZonerPurseLimit = RecZoner.Fields.Item("PurseLimitMM").Value
...
RecTrans.Source = "SELECT * FROM viewTediTransactionsMM Where TID = " & Request.QueryString("TID") & " Order by CID Desc"
...
RecTrans_cmd.CommandText = "SELECT * FROM viewTediTransactionsMM WHERE TID = ? ORDER BY CDate DESC" 
...
SaveFileName = RecZoner.Fields.Item("TediEmpCode").Value & "-Agent_MobileMoney_History-" & Day(Now) & Month(Now) & Year(Now) & "-" & Hour(Now) & Minute(Now) & Second(Now) & ".csv"
...
%>
                                <td class="action-td"><a href="UnallocateTransMM.asp?CID=<%=RecTrans.Fields.Item("CID").Value%>&TID=<%=Request.QueryString("TID")%>" class="clone-button"></a></td>
<%Else%><td>&nbsp;</td>
<%End If
%>
<%
...
%>
                                <td class="action-td"><a href="DeleteFNBTransMM.asp?CID=<%=RecTrans.Fields.Item("CID").Value%>&TID=<%=Request.QueryString("TID")%>" class="delete-button"></a></td>
<%Else%><td>&nbsp;</td>
<%End If
%>
...
<center><a href="TediMChargeHistoryFullMM.asp?TID=<%=Request.QueryString("TID")%>&Transtype=<%=Request.QueryString("Transtype")%>&Excel=Yes" class="nice red radius button">Export To Excel</a></center>
<%End If%>
