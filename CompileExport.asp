...
End If
		...
		RecInsert.Open "SELECT Top(1) * FROM BulkMCharge", MM_Site_STRINGWrite, 1, 2
		RecInsert.AddNew
...
RecNewestBulk.Source = "SELECT * FROM BulkMCharge Order By BulkID Desc"
...
SFile = Day(Now) & Month(Now)  & Year(Now) & ".BulkID" & NewestBulkID & ".AdminID" & UserSID
...
RecCurrent.Source = "SELECT * FROM ViewTediDetailWithTotals where TediActive = 'True' and RID = " & Request.Form("RID") & " and MChargeTedi = 'True' Order By TediFirstName Asc"
...
Response.flush
		...
		RecInsertTmp.Open "SELECT Top(1) * FROM BulkMChargeTediTemp", MM_Site_STRINGWrite, 1, 2
		RecInsertTmp.AddNew
...
function delayer(){
	...
	window.location = "CompileExportComplete.asp?BulkID=<%=NewestBulkID%>"
