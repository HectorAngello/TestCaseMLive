...
DashFileName = "DashboardSubRegions_" & Session("UNID") & "_" & Day(Now) & MonthName(Month(Now),true) & Year(Now) & "-" & Hour(Now) & Minute(Now) & Second(Now)
	...
	<div class="container">
        <div id="main-menu" class="row">
            <div class="three columns">
                <!-- #include file="Includes/sidebar.asp" --><br><a href="ViewRegBreakDown.asp?RID=<%=Request.QueryString("RID")%>&RegionYear=<%=Request.QueryString("RegionYear")%>&RegionMonth=<%=Request.QueryString("RegionMonth")%>" class="nice blue radius button">View <%=RecRegionalDash.Fields.Item("RegionCode").Value%> Mentors For The Same Period</a>
...
RecHCTarget.Source = "SELECT Top(1)* FROM  PrerenderSubRegionsDashboard2 Where SRID = " & SRID & " and RepMonth = " & RegionMonth & " and RepYear = " & RegionYear
...
TheFile.Writeline(MonthName(RegionMonth) & " " & RegionYear & "," & RecSubRegions.Fields.Item("SubRegionName").Value & "," & Formatnumber(CurrentHC,,,,0) & "," & Formatnumber(TargetHC,,,,0) & "," & Formatnumber(AirTimeTarget,,,,0) & "," & FormatNumber(AirtimeBanked,,,,0) & "," & AirtimePerc & "," & FormatNumber(VendsAmount,,,,0) & "," & FormatNumber(VendsAmountData,,,,0) & "," & FormatNumber(Deductions,,,,0) & "," & FormatNumber(ConnectionsTarget,,,,0) & "," & FormatNumber(ConnectionsToDate,,,,0) & "," & ConnectPerc)
%>


<tr>
	<td><a href="SubRegionBreakDown.asp?SRID=<%=RecSubRegions.Fields.Item("SRID").Value%>&Regionmonth=<%=Request.QueryString("Regionmonth")%>&RegionYear=<%=Request.QueryString("RegionYear")%>"><%=RC%>. <%=RecSubRegions.Fields.Item("SubRegionName").Value%></a></td>
