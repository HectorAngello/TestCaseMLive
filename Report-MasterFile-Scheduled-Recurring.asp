...
set RecReccurringType = Server.CreateObject("ADODB.Recordset")
RecReccurringType.ActiveConnection = MM_Site_STRING
RecReccurringType.Source = "Select * FROM ScheduledRecurringReportTypes Where ShedRecuringActive = 'True' order By ShedRecuringTypeID Asc"
RecReccurringType.CursorType = 0
RecReccurringType.CursorLocation = 2
RecReccurringType.LockType = 3
RecReccurringType.Open()
RecReccurringType_numRows = 0

%><table width="100%" border="0" cellspacing="2" cellpadding="2">

  <tr>
    <td align="left" valign="top"><form action="AddToScheduledReportRecurring.asp" name="ZRCS" method="Post" Target="_parent">
<table>
<tr><td colspan="2"><h4>Recurring Information:</h4></td></tr>
<tr><td>Run Report:</td><td>
<form name="form1" method="get" action="">
<select name="menu2" onChange="MM_jumpMenu2('this',this,0)" Class="text3_frm">
<Option Value="Report-MasterFile-Scheduled-Recurring.asp">Select Option</Option>
<%While Not RecReccurringType.EOF%>
<option value="Report-MasterFile-Scheduled-Recurring.asp?ShedRecuringTypeID=<%=(RecReccurringType.Fields.Item("ShedRecuringTypeID").Value)%>" <%If Cstr(RecReccurringType.Fields.Item("ShedRecuringTypeID").Value) = Request.QueryString("ShedRecuringTypeID") Then%>Selected<%End If%> ><%=(RecReccurringType.Fields.Item("ShedRecuringLabel").Value)%></option>
<%
RecReccurringType.MoveNext
Wend
%>
             </select>
...
<%
If Request.QueryString("ShedRecuringTypeID") <> "" Then
If Request.QueryString("ShedRecuringTypeID") = "2" Then
%>
<tr>
<td>Day of the week:</td><td><select Name="DayOfTheWeek">
<%
DayX = 0
Do While DayX < 7
DayX = DayX + 1
%>
<Option Value="<%=WeekDayName(DayX)%>"><%=WeekDayName(DayX)%></option>
<%
Loop
%>
</select></td>
...
<%
End If
If Request.QueryString("ShedRecuringTypeID") = "3" Then
%>
<tr>
<td>Day of the month:</td><td><select Name="DayOfTheMonth">
<%
DayX = 0
Do While DayX < 28
DayX = DayX + 1
%>
<Option Value="<%=(DayX)%>"><%=(DayX)%></option>
<%
Loop
...
End If
%>
<tr><td class="quote">Run Report At:</td><td>
...
<tr><td colspan="2"><h4>Report Criteria:</h4></td></tr>
<tr>
...
<input type="Hidden" Name="ReportRunID" Value="9">
<input type="Hidden" Name="OnceOffReportID" Value="1">
<input type="Hidden" Name="UNID" Value="<%=Session("UNID")%>">
<input type="hidden" Name="ShedRecuringTypeID" Value="<%=Request.QueryString("ShedRecuringTypeID")%>">
</form>
